...
export default function NewTaskForm({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-panel p-4 flex flex-col sm:flex-row gap-3">
      ...
      <div className="flex items-center gap-2 bg-gray-900/50 border border-aura-glassBorder rounded px-3 py-1 focus-within:border-aura-neonPurple focus-within:shadow-[0_0_10px_-2px_#b537f2] transition-all">
        <span className="text-xs text-gray-400 font-mono tracking-wider">Deadline</span>
        <input
          type="date"
          value={deadline}
